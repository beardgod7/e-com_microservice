...
class CartService {
    ...
    async addProduct(cartId, product) {
        ...
        products = products.map((p) => {
            if (p.productName === product.name) {
                ...
                productExists = true;
                return Object.assign(Object.assign({}, p), { quantity: p.quantity + 1, price: (p.price / p.quantity) * (p.quantity + 1) // Update the total price proportionally
                 });
            }
    ...
    async deleteCartItem(cartId, productName) {
        const cart = await this.cartModel.findByPk(cartId);
        if (!cart)
            return null;
        const productIndex = cart.products.findIndex((product) => product.productName === productName);
        if (productIndex > -1) {
